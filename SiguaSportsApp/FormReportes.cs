...
namespace SiguaSportsApp
{
    public partial class FormReportes : Form
    {
        ...
        string query = "SELECT case when v.num_factura is null then 'Total' else v.num_factura end Factura, SUM(vd.cantidad * vd.precioVenta)Total " +
            "FROM Ventas v inner join VentaDetalle vd on v.num_factura = vd.num_factura WHERE v.fecha_Venta = '"+DateTime.Today.ToShortDateString()+"' GROUP BY v.num_factura WITH ROLLUP ";
        string devolucion = "SELECT Case when dd.cod_estado = '1' then 'Devolucion' else 'Cambio' end Estado, COUNT(*) Cantidad FROM Devoluciones d " +
            "inner join DevolucionDetalle dd on d.num_devolucion = dd.num_devolucion where fecha_devolucion = '" + DateTime.Today.ToShortDateString() + "' group by dd.cod_estado order by Estado ";
